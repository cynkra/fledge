...
unbump_version_impl <- function() {
  ...
  if (fledge_chatty()) cli_alert_info("Checking if working copy is clean.")
  stopifnot(sum(map_int(gert::git_status(), length)) == 0)
  if (fledge_chatty()) cli_alert_info("Checking if last tag points to last commit.")
  stopifnot(tag_last_commit$commit == last_commit$commit)
  if (fledge_chatty()) cli_alert_info("Checking if commit messages match.")
  stopifnot(is_last_commit_bump())

  if (fledge_chatty()) cli_alert_success("Safety checks complete.")

  if (fledge_chatty()) cli_alert("Deleting tag {.field {tag$name}}.")
