...
as an example. We are going to call our package “{tea}”. We will develop
it from scratch and also maintain a changelog as the development
...
We are typing this demo as an R Markdown vignette therefore we will be
using R tools for creating files, editing them, and interacting with
git: in real life you can be using e.g. an IDE or the command line. The
fledge package won’t care!

## Set up the development environment

Before we start development for {tea}, we set up the basic development
environment required for any typical R package.
...
In an interactive RStudio session, a new window opens and you would work
there. Users of other environments would change the working directory
...
Next, one would set up this package for development and create a Git
repository for the package. We achieved this with gert code.

You could use
[`usethis::use_git()`](https://usethis.r-lib.org/reference/use_git.html)
function that creates an initial commit, and the repository is in a
clean state.
...
We also set up a git remote. In real life you might be using a function
like
...
We create two functions to show the contents and tags of the remote. In
real life, you’d probably simply browse the GitHub interface for
instance!
...
creating the new R file called `cup.R` and adding code (well only a
comment).
...
That is our first fledge specific step! Note the **use of the bullet (-)
at the beginning of the commit message**. This indicates that the
message should be included in `NEWS.md` when it is updated.

It does not matter how and where you type the commit message (gert in R,
RStudio Git window, command line, VSCode, etc.). What’s important is the
...
The code in `cup.R` warrants a test (at least it would if it were actual
code!):
...
Let us look at the commit history until now. You might use any Git tool
...
to assign a new dev version number to the package and also update
`NEWS.md`.
...
If you run
[`fledge::bump_version()`](https://fledge.cynkra.com/dev/reference/bump_version.md)
too early by mistake (e.g. you wanted to do one more code edit), you can
run
[`fledge::unbump_version()`](https://fledge.cynkra.com/dev/reference/unbump_version.md)!
This should happen immediately after bumping. If you have pushed or
edited code in the meantime, it’s too late – just continue and assign a
new version when you are done with the edits.
...
(congrats if you noticed right away that we typed “tee” instead of
“tea”!).

The fledge package adds a comment about not editing `NEWS.md` by hand to
`NEWS.md` but actually you can… if you do it right! Read on.
...
([Editing commit
messages](https://docs.github.com/en/github/committing-changes-to-your-project/creating-and-editing-commits/changing-a-commit-message#amending-older-or-multiple-commit-messages)
is not something fledge supports).
...
and not to commit manually.

Using
[`fledge::finalize_version()`](https://fledge.cynkra.com/dev/reference/finalize_version.md)
instead of committing manually ensures that the tag is set to the
correct version in spite of the `NEWS.md` update. It should be called
when `NEWS.md` is manually updated. Note that it should be called after
[`fledge::bump_version()`](https://fledge.cynkra.com/dev/reference/bump_version.md),
...
A tag has been created for the version which is good practice, and
crucial when using fledge: for updating the changelog, fledge looks
through all commit messages **since the latest tag**.

### Change code and commit

{tea} with cup is tested, now we want to enhance with bowl. This
...
Now that we have added bowl support to our package, it is time to bump
the version again.
...
It seems we do not even need to amend the `NEWS.md` by hand this time as
we made no typo!
...
A difference between publishing on CRAN and publishing on GitHub is that
there’s an external system controlling your publication on CRAN so your
package might need some tweaks between the first release candidate and
what version you end up publishing on CRAN.

### Bump version for release

We wish to release this package as a patch and so we use
[`fledge::bump_version()`](https://fledge.cynkra.com/dev/reference/bump_version.md)
with the “patch” argument. Other values for the arguments are “dev”
...
changelog to meaningful release notes. E.g. in real life we might
[re-organize
bullets](https://style.tidyverse.org/news.html#organisation-3).

Unlike with development versions, we commit the changes to `NEWS.md`
manually, not with a fledge function.
...
Some time passed and our {tea} package was accepted on CRAN. At this
...
[`fledge::tag_version()`](https://fledge.cynkra.com/dev/reference/tag_version.md)
function.
...
Push to remote, add features with relevant commits (after mergining a
branch or not),
