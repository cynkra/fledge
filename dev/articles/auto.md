...
1.  Avoid conflicts in `NEWS.md` and `DESCRIPTION`
    - Bumping to CRAN version and back to dev version is atomic, no
      dangling release branch
    - Support for subsequent release attempts off of a branch created
      from a tag
2.  Easy to undo a release attempt
    - Pre-flight with `plan_release("pre-patch")`, including grooming of
      `NEWS.md`
    - Caveat: once committed to a release, there is no way back, to
      avoid conflicts
...
4.  Minimize interactive wait time
    ...
    - TODO: Use background jobs via
      [`job::empty()`](https://lindeloev.github.io/job/reference/job.html)
      where useful
...
9.  Consistency with
    [`usethis::use_release_issue()`](https://usethis.r-lib.org/reference/use_release_issue.html)
    - TODO: snapshot tests for `usethis:::release_checklist()`, maintain
      own list

## Non-goals

1.  Always release from main branch
    - Conflicts with important goal to avoid conflicts, and also with
      auto-merge and merge queues
    - Release from main branch possible in some cases

## Workflow

1.  Optional, recommended because grooming the NEWS often gives a hint
    about what’s missing in a release:

    - `fledge::plan_release("pre-patch")` (or `"pre-minor"` or
      `"pre-major"`)
...
2.  Mandatory:
    ...
    - [`fledge::release()`](https://fledge.cynkra.com/dev/reference/release.md)
