...
pkg_dir <- file.path(tempdir(check = TRUE), "fledge")
dir.create(pkg_dir)
pkg <- fledge::create_fledge_mock_repo("tea", open = FALSE, tempdir = pkg_dir)
```
...
gert::git_commit("- Add support for a cup of tee.")
...
The current version number of our package is `r desc::desc_get_version(pkg)`.

```{r bump}
fledge::bump_version()
```

The new version number is `r desc::desc_get_version(pkg)`.
...
gert::git_commit("- Bowl of tea enabled.")
```
...
```{r end, pkg=FALSE, eval=FALSE}
# I need to get smarter about temporary directories Rmd etc.
usethis::local_project(path = dirname(knitr::current_input(dir = TRUE)))
