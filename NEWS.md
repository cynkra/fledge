...
- Cynkratemplate::use_cynkra_pkgdown() + re-add comment (@krlmlr,
  @41898282+github-actions\[bot\], @maelle, #568).
...
- New default `pre_release(which = "next")` (#521,
  #524).
...
- Extract contributor name from PR merge message (@krlmlr,
  #358)

- Harvest PR attribution from squash commits (@maelle, #349)

- Harvest PR title from merge commit messages (@krlmlr, @maelle,
  #343)
...
- Improve bump_version() behavior in the absence of changes (@krlmlr,
  @maelle, #323)
...
- Update summary of how fledge uses commit messages (#499,
  #511).
...
- New `pre_release()`, `release()` and `post_release()` for
...
4.  Depending on the kind of update:
    - If "dev", `finalize_version()`: commit `DESCRIPTION` and
      `NEWS.md`, create tag with message
    - Otherwise, `commit_version()`; the user needs to call
...
can also edit `NEWS.md` and call `tag_version()` again. Both the commit
...
current date in `update_version()`. Likewise, if `NEWS.md` contains
dates in the headers, new versions also get a date.
...
Also includes helper functions `get_last_tag()` and
